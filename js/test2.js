class StreamingSearch {
  constructor() {
    ...
    this.API_POST_ITEMS = window.location.origin + '/wp-json/wp/v2/posts';
    this.API_BLOGS = window.location.origin + '/wp-json/wp/v2/blogs';
  ...
  async fetchAllData() {
    try {
      console.log('📡 Fetching data from:', {
        posts: this.API_POST_ITEMS + '?per_page=100',
        blogs: this.API_BLOGS + '?per_page=100'
      });
      ...
      if (!postItemsRes.ok || !blogsRes.ok) {
        throw new Error(`Failed to fetch data: Posts ${postItemsRes.status}, Blogs ${blogsRes.status}`);
      }
      ...
      console.log('Data loaded successfully:', {
        streamingSites: this.state.allPostItems.length,
        blogs: this.state.allBlogs.length
      });
    } catch (error) {
  ...
  performSearch(query) {
    ...
    console.log('✅ Search results:', {
      streamingSites: filteredPostItems.length,
  ...
  renderBlogs() {
    const html = this.state.blogResults.map((blog, index) => {
      ...
      const desc = blog.meta?.bg_short_desc || 'No description available';
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 Initializing StreamingSearch...');
  new StreamingSearch();
});